...
class CheckUserCertificates {
	...
	public function description(): string {
		$configValue = $this->config->getAppValue('files_external', 'user_certificate_scan', false);
		if ($configValue === false) {
			return '';
		}
		if ($configValue === 'not-run-yet') {
	...
	public function elements(): array {
		$configValue = $this->config->getAppValue('files_external', 'user_certificate_scan', false);
		if ($configValue === false || $configValue === 'not-run-yet') {
			return [];
		}
		$data = json_decode($configValue);
		if (!is_array($data)) {
			return [];
		}
		return $data;
